...
class AnnaDataset(Dataset):
    ...
    def __len__(self):
        return int(len(self.integer_encoded-1)/self.len_seq)

    def __getitem__(self, idx):
        x = self.onehot_encoded[idx*self.len_seq:(idx+1)*self.len_seq,:]
        y = self.integer_encoded[(idx+1)*self.len_seq]
        return  x,  y
...
def IntcodeAnna(X):
    ...
    label_encoder = LabelEncoder()
    label_encoder.fit(categories)
    intchar = label_encoder.transform(list(X))
...
def HotcodeAnna(X):
    ...
    onehot_encoder = OneHotEncoder(sparse=False)
    onehot_encoder.fit(np.arange(len(label_encoder.classes_)).reshape(-1, 1))
    onehot_encoded = torch.FloatTensor(onehot_encoder.transform(X))
